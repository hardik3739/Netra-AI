...
export default function App() {
  // --- CORE APP STATES ---
  const [activeSidebarTab, setActiveSidebarTab] = useState<SidebarTab>("AUDIT_TRAIL");
  ...
  // Starts around 1.2M and dynamically increments when API stream is ticking
  ...
  // --- REAL-TIME LIVE DATA COMMIT SIMULATOR ---
  useEffect(() => {
    if (!apiActive) return;
    ...
    const interval = setInterval(() => {
      ...
      // Randomly trigger standard incremental audits (15% chance per tick)
      if (Math.random() < 0.15) {
        setAiAudits((prev) => prev + 1);
      }

      // Randomly inject a real automatic system ledger log block to feel truly alive! (6% chance per tick)
      if (Math.random() < 0.06) {
        const testVectors = [
